...
- **Video/Audio RAG** 
    Video/Audio Retrieval using Retrieval-Augmented Generation (RAG) is an advanced approach that combines traditional video/audio retrieval methods with generative AI techniques. This integration aims to improve how users search for, retrieve, and interact with video/audio content.
...
- **Logic of Thoughts**
    Logic-of-Thought (LoT) prompting employs propositional logic to generate expanded logical information from input context, and utilizes the generated logical information as an additional augmentation to the input prompts, thereby enhancing the capability of logical reasoning.
...
2. Locate the `.env` file, which contains keys like the following:
    ...
    GOOGLE_API_KEY= "${googleapikey}" # Google gemini api key if using Gemini
    GOOGLE_MODEL= "${googlemodel}" # google gemini model name if using Gemini
    AWS_SERVICE_NAME = "${awsservicename}"
    AWS_MODEL_ID="${awsmodelid}"
    AWS_REGION = "${awsregion}"
    AWS_ACCESS_KEY_ID = "${awsaccesskeyid}"
    AWS_SECRET_ACCESS_KEY = "${awssecretaccesskey}"
    AWS_SESSION_TOKEN = "${awsessiontoken}"
    ...
    AWS_SERVICE_NAME = "your-service-name"
...
NOTE: You can choose either openai, gemini or aws bedrock configurations.
...
NOTE: Use "llmtype" in the API payload to choose a particular LLM provider you want to use. 
Ensure to use the same LLM provider for fileupload api and Retrievalkeplar api. For example, if you use "gemini" for fileupload api, also use "gemini" for Retrievalkeplar, COT, THOT, COV, LOT. This is because the embedding model provider and the chat generation model providers should be the same.

## Compatible Models
The codebase is tested to be compatible with the following models: Azure OpenAI (gpt-4o-mini), Google Gemini (gemini-2.0-flash, gemini-2.5-flash-preview-04-17, gemini-2.5-pro-preview-03-25), AWS Bedrock (claude-3-sonnet)
